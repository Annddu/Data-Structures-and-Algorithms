...
class FixedCapBiMap {
	...
	private:
		//TODO - Representation
		TElem* elements; //the begining of a chunk of memory
		int capacity; //maximum elements we want to store, it is set at the beginning of elements and it is not going to change, we cant let the mapSize increase past the capacity
		int mapSize; //how many elements are currently, update when add, delete etc. in the map
	...
	//removes a key k with all the values that belong to it. Returns a ValuePair containing the removed value in the following way
